...
// PDF Preview Screen with Save Option
class PDFPreviewScreen extends StatelessWidget {
  ...
  void _savePdf(BuildContext context) async {
    ...
    final savedPath = "${downloadsDir!.path}/Invoice_${DateTime.now().millisecondsSinceEpoch}.pdf";
    File(pdfPath).copySync(savedPath);
...
class PdfService {
  static Future<String> generateInvoice(
    ...
    final headingStyle = pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold, color: PdfColors.blue);
...
//             // Table for Items
  ...
  static double _calculateTotal(List<List<String>> items) {
    return items.fold(0, (total, item) => total + (double.tryParse(item[2]) ?? 0));
  }
}